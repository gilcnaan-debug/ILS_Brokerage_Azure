...
# MAGIC create or replace view bronze.All_Relay_Loads_Genai
# MAGIC  as (
# MAGIC  
...
# MAGIC
# MAGIC
# MAGIC  ,starting_data AS (
# MAGIC   SELECT DISTINCT 
# MAGIC     b.relay_reference_number,
# MAGIC     COALESCE(master_date.pu_appt_date, master_date.ready_date) AS pu_appt,
# MAGIC     master_date.pickup_datetime,
# MAGIC     master_date.del_appt_date,
# MAGIC     master_date.delivery_datetime,
# MAGIC     new_office_lookup.new_office AS cust_office,
# MAGIC     customer_profile_projection.customer_name,
# MAGIC     relay_users.full_name AS acct_mgr,
# MAGIC     sales.full_name AS salesrep,
# MAGIC     'TL' AS mode,
# MAGIC     COALESCE(tendering_service_line.service_line_type, tsl.service_line_type) AS service_line_type,
# MAGIC     COALESCE(combined_loads_mine.total_load_rev, relay_customer_money.final_customer_rate) AS cust_rate,
# MAGIC     COALESCE(combined_loads_mine.total_load_lhc_rev, relay_customer_money.customer_lhc) AS cust_lhc,
# MAGIC     COALESCE(combined_loads_mine.total_load_fuel_rev, relay_customer_money.customer_fsc) AS cust_fsc,
# MAGIC     COALESCE((combined_loads_mine.total_load_rev - combined_loads_mine.total_load_lhc_rev - combined_loads_mine.total_load_fuel_rev), relay_customer_money.customer_acc) AS cust_acc,
# MAGIC     relay_carrier_money.total_carrier_rate,
# MAGIC     relay_carrier_money.lhc_carrier_rate,
# MAGIC     relay_carrier_money.fsc_carrier_rate,
# MAGIC     relay_carrier_money.acc_carrier_rate,
# MAGIC     COALESCE(combined_loads_mine.total_load_rev, relay_customer_money.final_customer_rate, 0) - COALESCE(relay_carrier_money.total_carrier_rate, 0) AS margin,
# MAGIC     b.booked_by_name,
# MAGIC     new_office_lookup_w_tgt.new_office AS booked_office,
# MAGIC     tendering_acceptance.shipment_id,
# MAGIC     tendering_acceptance.accepted_at,
# MAGIC     tender_reference_numbers_projection.nfi_pro_number AS customer_ref_num,
# MAGIC     COALESCE(t.trailer_number, b.trailer_number) AS trailer_number,
# MAGIC     master_date.booked_at AS master_booked_at,
# MAGIC     CASE 
# MAGIC       WHEN CAST(master_date.booked_at AS DATE) < CAST(COALESCE(master_date.pu_appt_date, master_date.ready_date) AS DATE) 
# MAGIC       THEN 'prebook' 
# MAGIC       ELSE 'sameday' 
# MAGIC     END AS category,
# MAGIC     b.first_shipper_name,
# MAGIC     INITCAP(b.first_shipper_city) AS origin_city,
# MAGIC     b.first_shipper_state AS origin_state,
# MAGIC     b.first_shipper_zip AS origin_zip,
# MAGIC     b.receiver_name,
# MAGIC     INITCAP(b.receiver_city) AS dest_city,
# MAGIC     b.receiver_state AS dest_state,
# MAGIC     b.receiver_zip AS dest_zip,
# MAGIC     b.booked_carrier_name,
# MAGIC     b.stop_count,
# MAGIC     COALESCE(t.status, b.status) AS tracking_status,
# MAGIC     COALESCE(tendering_planned_distance.planned_distance_amount, 0) AS miles,
# MAGIC     b.cargo_value_amount,
# MAGIC     b.booking_id,
# MAGIC     b.total_pallets,
# MAGIC     b.total_weight
# MAGIC   FROM 
# MAGIC     booking_projection b 
# MAGIC   JOIN 
# MAGIC     master_date ON b.relay_reference_number = master_date.relay_reference_number AND b.status = master_date.status 
# MAGIC   JOIN 
# MAGIC     analytics.dim_financial_calendar ON CAST(master_date.use_date AS DATE) = CAST(analytics.dim_financial_calendar.date AS DATE) 
# MAGIC   JOIN 
# MAGIC     customer_profile_projection ON b.tender_on_behalf_of_id = customer_profile_projection.customer_slug 
# MAGIC   LEFT JOIN 
# MAGIC     new_office_lookup ON customer_profile_projection.profit_center = new_office_lookup.old_office
# MAGIC   LEFT JOIN 
# MAGIC     relay_users ON customer_profile_projection.primary_relay_user_id = relay_users.user_id AND relay_users.`active?` = 'true'
# MAGIC   LEFT JOIN 
# MAGIC     relay_users sales ON customer_profile_projection.sales_relay_user_id = sales.user_id AND sales.`active?` = 'true'
# MAGIC   LEFT JOIN 
# MAGIC     relay_users book ON UPPER(b.booked_by_name) = UPPER(book.full_name) AND book.`active?` = 'true'
# MAGIC   LEFT JOIN 
# MAGIC     new_office_lookup_w_tgt ON book.office_id = new_office_lookup_w_tgt.old_office 
# MAGIC   LEFT JOIN 
# MAGIC     tendering_service_line ON b.relay_reference_number = tendering_service_line.relay_reference_number
# MAGIC   LEFT JOIN 
# MAGIC     combined_loads_mine ON b.relay_reference_number = combined_loads_mine.combine_new_rrn
# MAGIC   LEFT JOIN 
# MAGIC     tendering_service_line tsl ON combined_loads_mine.relay_reference_number_one = tsl.relay_reference_number
# MAGIC   LEFT JOIN 
# MAGIC     relay_carrier_money ON b.relay_reference_number = relay_carrier_money.relay_reference_number AND relay_carrier_money.status = 'booked'
# MAGIC   LEFT JOIN 
...
# MAGIC   LEFT JOIN 
# MAGIC     aljex_user_report_listing ON UPPER(b.booked_by_name) = UPPER(aljex_user_report_listing.full_name)
# MAGIC   LEFT JOIN 
# MAGIC     tendering_acceptance ON b.relay_reference_number = tendering_acceptance.relay_reference_number AND tendering_acceptance.`accepted?` = 'true'
# MAGIC   LEFT JOIN 
# MAGIC     truckload_projection t ON b.booking_id = t.booking_id AND t.status != 'CancelledOrBounced'
# MAGIC   LEFT JOIN 
# MAGIC     tendering_planned_distance ON b.relay_reference_number = tendering_planned_distance.relay_reference_number
# MAGIC   LEFT JOIN 
# MAGIC     tender_reference_numbers_projection ON b.relay_reference_number = tender_reference_numbers_projection.relay_reference_number
# MAGIC   WHERE 
# MAGIC     1 = 1 
# MAGIC     -- [[AND {{date_range}}]]
# MAGIC     -- [[AND {{customer}}]]
...
# MAGIC     -- [[AND {{acct_mgr}}]]
# MAGIC     -- [[AND {{booked_by}}]]
# MAGIC     -- [[AND CASE WHEN b.relay_reference_number IS NULL THEN 'LTL' ELSE 'TL' END = {{modee}}]]
# MAGIC     AND b.status = 'booked'
# MAGIC   
# MAGIC   UNION 
# MAGIC
# MAGIC   SELECT DISTINCT 
# MAGIC     e.load_number AS load_number,
# MAGIC     master_date.pu_appt_date,
# MAGIC     master_date.pickup_datetime,
# MAGIC     master_date.del_appt_date,
# MAGIC     master_date.delivery_datetime,
# MAGIC     CASE 
# MAGIC       WHEN customer_id = 'hain' THEN 'PHI'
# MAGIC       WHEN customer_id = 'roland' THEN 'LTL'
# MAGIC       WHEN customer_id = 'deb' THEN 'PHI' 
# MAGIC       ELSE new_office_lookup.new_office 
# MAGIC     END AS cust_office,
# MAGIC     CASE 
# MAGIC       WHEN customer_id = 'hain' THEN 'Hain c/o NFIL 3G'
...
# MAGIC     sales.full_name AS salesrep,
# MAGIC     'LTL' AS mode,
# MAGIC     tendering_service_line.service_line_type,
# MAGIC     relay_customer_money.final_customer_rate,
# MAGIC     relay_customer_money.customer_lhc AS cust_lhc,
# MAGIC     relay_customer_money.customer_fsc AS cust_fsc,
# MAGIC     relay_customer_money.customer_acc,
# MAGIC     e.projected_expense / 100.00 AS total_carrier_rate,
# MAGIC     e.carrier_linehaul_expense / 100.00 AS carrier_linehaul_expense,
# MAGIC     e.carrier_fuel_expense / 100.00 AS carrier_fuel_expense,
# MAGIC     e.carrier_accessorial_expense / 100.00 AS carrier_accessorial_expense,
# MAGIC     COALESCE(relay_customer_money.final_customer_rate, 0) - COALESCE(e.projected_expense / 100.00, 0) AS margin,
# MAGIC     'ltl_team' AS ltl_team,
# MAGIC     'LTL' AS ltl_mode,
# MAGIC     tendering_acceptance.shipment_id,
# MAGIC     tendering_acceptance.accepted_at,
# MAGIC     tender_reference_numbers_projection.nfi_pro_number AS customer_ref_num,
# MAGIC     t.trailer_number,
# MAGIC     NULL AS booked_at,
# MAGIC     NULL AS category, -- No booked at date for LTL relay
# MAGIC     e.pickup_name,
# MAGIC     INITCAP(e.pickup_city) AS pickup_city,
# MAGIC     e.pickup_state,
# MAGIC     e.pickup_zip,
# MAGIC     e.consignee_name,
# MAGIC     INITCAP(e.consignee_city) AS consignee_city,
# MAGIC     e.consignee_state,
# MAGIC     e.consignee_zip,
# MAGIC     e.carrier_name,
# MAGIC     NULL AS stop_count,
# MAGIC     COALESCE(t.status, e.dispatch_status) AS tracking_status,
# MAGIC     COALESCE(tendering_planned_distance.planned_distance_amount, e.miles, 0) AS miles,
# MAGIC     NULL AS null1,
# MAGIC     NULL AS null2,
# MAGIC     e.pallet_count,
# MAGIC     e.weight
# MAGIC
# MAGIC   FROM 
# MAGIC     big_export_projection e 
# MAGIC   JOIN 
# MAGIC     master_date ON e.load_number = master_date.booking_id AND e.dispatch_status = master_date.status 
# MAGIC   JOIN 
# MAGIC     analytics.dim_financial_calendar ON CAST(master_date.use_date AS DATE) = CAST(analytics.dim_financial_calendar.date AS DATE) 
# MAGIC   LEFT JOIN 
# MAGIC     customer_profile_projection ON e.customer_id = customer_profile_projection.customer_slug 
# MAGIC   LEFT JOIN 
# MAGIC     new_office_lookup ON CASE 
# MAGIC       WHEN customer_id IN ('deb','hain') THEN 'NJ'
# MAGIC       WHEN customer_id = 'roland' THEN 'LT' 
# MAGIC       ELSE customer_profile_projection.profit_center 
...
# MAGIC     relay_users sales ON customer_profile_projection.sales_relay_user_id = sales.user_id AND sales.`active?` = 'true'
# MAGIC   LEFT JOIN 
# MAGIC     tendering_service_line ON e.load_number = tendering_service_line.relay_reference_number
# MAGIC   LEFT JOIN 
# MAGIC     relay_customer_money ON e.load_number = relay_customer_money.relay_reference_number AND e.dispatch_status = relay_customer_money.status 
# MAGIC   LEFT JOIN 
# MAGIC     aljex_user_report_listing ON CASE 
# MAGIC       WHEN e.load_number IS NOT NULL THEN 'ltl_team' 
# MAGIC       ELSE 'abc' 
# MAGIC     END = aljex_user_report_listing.full_name 
# MAGIC   LEFT JOIN 
# MAGIC     tendering_acceptance ON e.load_number = tendering_acceptance.relay_reference_number AND tendering_acceptance.`accepted?` = 'true'
# MAGIC   LEFT JOIN 
# MAGIC     truckload_projection t ON e.load_number = t.relay_reference_number AND t.status != 'CancelledOrBounced'
# MAGIC   LEFT JOIN 
# MAGIC     tendering_planned_distance ON e.load_number = tendering_planned_distance.relay_reference_number
# MAGIC   LEFT JOIN 
# MAGIC     tender_reference_numbers_projection ON e.load_number = tender_reference_numbers_projection.relay_reference_number
# MAGIC   WHERE 
# MAGIC     1 = 1 
# MAGIC     -- [[AND {{date_range}}]]
# MAGIC     -- [[AND {{customer}}]]
...
# MAGIC     -- [[AND {{acct_mgr}}]]
# MAGIC     -- [[AND {{booked_by}}]]
# MAGIC     -- [[AND CASE WHEN e.load_number IS NULL THEN 'TL' ELSE 'LTL' END = {{modee}}]]
# MAGIC     AND e.dispatch_status != 'Cancelled'
# MAGIC     AND e.carrier_name IS NOT NULL
# MAGIC )
# MAGIC ,final_code as ( SELECT DISTINCT 
# MAGIC   relay_reference_number AS `Load_Num`,
# MAGIC   tracking_status AS `Status`,
# MAGIC   cust_office AS `Office`,
# MAGIC   customer_name AS `Customer`,
# MAGIC   acct_mgr AS `Acct_Mgr`,
# MAGIC   salesrep AS `Sales_Rep`,
# MAGIC   mode AS `Mode`,
# MAGIC   service_line_type AS `Equipment`,
# MAGIC   booking_id AS `Booking_ID`,
# MAGIC   shipment_id AS `Shipment_ID`,
# MAGIC   customer_ref_num AS `Cust_Ref_Num`,
# MAGIC   cargo_value_amount AS `Cargo_Value_Amt`,
# MAGIC   accepted_at AS `Tendered_At`,
# MAGIC   booked_carrier_name AS `Carrier`,
# MAGIC   booked_by_name AS `Booked_By`,
# MAGIC   booked_office AS `Booked_By_Office`,
# MAGIC   master_booked_at AS `Booked_At`,
# MAGIC   category AS `Prebooked`,
# MAGIC   pu_appt AS `PU_Appt`,
# MAGIC   pickup_datetime AS `Actual_PU`,
# MAGIC   del_appt_date AS `Del_Appt`,
# MAGIC   delivery_datetime AS `Actual_Delivery`,
# MAGIC   first_shipper_name AS `Origin`,
# MAGIC   origin_city AS `Origin_City`,
# MAGIC   origin_state AS `Origin_State`,
# MAGIC   origin_zip AS `Origin_Zip`,
# MAGIC   receiver_name AS `Destination`,
# MAGIC   dest_city AS `Dest_City`,
# MAGIC   dest_state AS `Dest_State`,
# MAGIC   dest_zip AS `Dest_Zip`,
# MAGIC   miles AS `Miles`,
# MAGIC   stop_count AS `Stop_Count`,
# MAGIC   cust_rate AS `Total_Rev`,
# MAGIC   cust_lhc AS `Cust_LHC`,
# MAGIC   cust_fsc AS `Cust_FSC`,
# MAGIC   cust_acc AS `Cust_ACC`,
# MAGIC   total_carrier_rate AS `Total_Exp`,
# MAGIC   lhc_carrier_rate AS `Carr_LHC`,
# MAGIC   fsc_carrier_rate AS `Carr_FSC`,
# MAGIC   acc_carrier_rate AS `Carr_ACC`,
# MAGIC   margin AS `Total_Margin`,
# MAGIC   CASE 
# MAGIC     WHEN cust_rate IS NULL THEN 0 
# MAGIC     WHEN CAST(cust_rate AS FLOAT) = 0 THEN 0 
# MAGIC     ELSE CAST(margin AS FLOAT) / CAST(cust_rate AS FLOAT) 
# MAGIC   END AS `Margin_Percent`,
# MAGIC   total_pallets as Pallets_Count,
# MAGIC   total_weight as Weight
# MAGIC FROM starting_data )
# MAGIC
# MAGIC select * from final_code
# MAGIC -- where PU_Appt like "%2024-12-10%" 
# MAGIC
# MAGIC
# MAGIC   
# MAGIC
...
# MAGIC %sql
# MAGIC
# MAGIC truncate table analytics.All_Relay_Loads_Genai_Temp;
# MAGIC INSERT INTO analytics.All_Relay_Loads_Genai_Temp
# MAGIC SELECT *
# MAGIC FROM bronze.All_Relay_Loads_Genai

# COMMAND ----------

# MAGIC %sql
# MAGIC
# MAGIC truncate table analytics.All_Relay_Loads_Genai;
# MAGIC INSERT INTO analytics.All_Relay_Loads_Genai
# MAGIC SELECT *
# MAGIC FROM analytics.All_Relay_Loads_Genai_Temp
