...
ErrorLogger = ErrorLogs("NFI_Shipper_Mapping_Exception")
...
try:    
    ...
    client_email = 'google-sheet-api@axial-reference-461717-q3.iam.gserviceaccount.com' 
    # Create credentials object
    credentials_dict = {
    "type": "service_account",
    "project_id": "axial-reference-461717-q3",
    "private_key_id": "a71dd1351dd3a0fbc9360f5a2227cf8b9be1b13b",
    "private_key": private_key,
    "client_email": client_email,
    "client_id": "114957970332827928381",
    "auth_uri": "https://accounts.google.com/o/oauth2/auth",
    "token_uri": "https://oauth2.googleapis.com/token",
    "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
    "client_x509_cert_url": f"https://www.googleapis.com/robot/v1/metadata/x509/{client_email}",
    ...
    if len(worksheets) > 2:
        ...
        if records:  # If the sheet has data
            ...
            df_main = df_spark.withColumn(
                "merge_key",
                concat_ws("|", col('Customer Master'))
            )
            df_sheet3 = df3_spark.withColumn(
                "merge_key",
                concat_ws("|", df3_spark["Customer Master"])
            )
    ...
    for i in range(2, len(data_to_insert) + 1):
        try:
            ...
            if "429" in str(e):
                print("Quota exceeded. Waiting for 60 seconds before retrying...")
                time.sleep(60)
    print("Data inserted into exceptions sheet and formatted successfully!")
