...
# MAGIC * **Created Date:** 06/09/2024
# MAGIC * **Created By:** Hariharan
# MAGIC * **Modified Date:** 06/09/2024
...
# MAGIC 	create or replace view bronze.vw_genai_fact_load_v1_vw as
...
# MAGIC 	), ---Select * from aljex_data where use
...
# MAGIC 		  - coalesce(carrier_charges.total_carrier_cost, 0)::float
# MAGIC 		)
# MAGIC 		/ 2 as Margin,
# MAGIC 		carrier_charges.carr_lhc / 2 as Linehaul,
# MAGIC 		carrier_charges.carr_fsc / 2 as Fuel_Surcharge,
# MAGIC 		carrier_charges.carr_acc / 2 as Other_Fess,
# MAGIC 		salesrep,
# MAGIC 		Trailer_Num,
...
# MAGIC 		use_relay_loads.Mode,
# MAGIC 		use_relay_loads.Equipment,
# MAGIC 		-- use_relay_loads.modee as Equipment,
# MAGIC 		date_range_two.week_num,
# MAGIC 		Case
# MAGIC 		  when customer_office = 'TGT' then 'PHI'
# MAGIC 		  else customer_office
...
# MAGIC 		total_miles::float,
# MAGIC 		carrier_charges.total_carrier_cost::float / 2 as split_rev,
# MAGIC 		total_carrier_rate.total_carrier_rate::float / 2::float as split_exp,
# MAGIC 		coalesce(total_cust_rate.total_cust_rate, 0)::float
# MAGIC 		- coalesce((carrier_charges.total_carrier_cost::float / 2), 0)::float / 2 as split_margin,
# MAGIC 		carrier_charges.carr_lhc / 2 as Linehaul,
# MAGIC 		carrier_charges.carr_fsc / 2 as Fuel_Surcharge,
# MAGIC 		carrier_charges.carr_acc / 2 as Other_Fess,
# MAGIC 		salesrep,
...
# MAGIC 		aljex_data
# MAGIC 		  left join bronze.date_range_two
# MAGIC 			on use_date::date = date_range_two.date_date::date --left join  bronze.load_pickup on use_relay_loads.relay_load  = load_pickup.Load_number
# MAGIC 	  UNION
# MAGIC 	  select distinct -- Carrier
# MAGIC 		id::float as relay_ref_num,
# MAGIC 		"0.5" as Load_Counter,
...
# MAGIC   left join pricing_nfi_load_predictions mr on ordered.Load_Number = mr.MR_id   ------ Market rate 
# MAGIC 	  where
# MAGIC 		rank = 1
# MAGIC 	  union
# MAGIC 	  select distinct -- Carrier
# MAGIC 		loadnum::float as Load_Number,
# MAGIC 		"1.0" as Load_Counter,
# MAGIC 		CASE
# MAGIC 		  WHEN
# MAGIC 			CAST(arrive_pu_date AS DATE) = '1900-01-01'
# MAGIC 		  THEN
# MAGIC 			CASE
# MAGIC 			  WHEN CAST(depart_pu_date AS DATE) = '1900-01-01' THEN CAST(pu_appt AS DATE)
# MAGIC 			  ELSE CAST(depart_pu_date AS DATE)
# MAGIC 			END
# MAGIC 		  ELSE CAST(arrive_pu_date AS DATE)
# MAGIC 		END AS ship_date,
...
# MAGIC 		--carr_office as office,
# MAGIC 		'Both',
# MAGIC 		'Digital Brokerage Group' AS Carrier_Rep,
# MAGIC 		CASE
# MAGIC 		  WHEN master_customer_name IS NULL THEN shipper
# MAGIC 		  ELSE master_customer_name
# MAGIC 		END AS mastername,
# MAGIC 		transfix_data.shipper as customer_name,
...
# MAGIC 		transfix_data.total_exp::float as split_exp,
# MAGIC 		COALESCE(total_rev, 0) - COALESCE(total_exp, 0) AS split_margin,
...
# MAGIC 				ELSE CAST(arrive_pu_date AS DATE)
# MAGIC 			  END
# MAGIC 			) = CAST(date_range_two.date_date AS DATE)
...
# MAGIC 				ELSE CAST(arrive_pu_date AS DATE)
# MAGIC 			  END
# MAGIC 			) = CAST(analytics.dim_financial_calendar.date AS DATE)
# MAGIC 		  LEFT JOIN customer_lookup
# MAGIC 			ON LEFT(shipper, 32) = LEFT(aljex_customer_name, 32)
# MAGIC 	  union
# MAGIC 	  SELECT
# MAGIC 		relay_ref_num,
# MAGIC 		Load_Counter,
...
# MAGIC 		cs.AdminFees_Cust / 2 as AdminFees_Cust,
# MAGIC 		cs.TransitandRouting_Cust / 2 as TransitandRouting_Cust,
# MAGIC 		cs.DelandPickup_Cust / 2 as DelandPickup_Cust,
# MAGIC 		cs.EquipmentandVehicle_Cust / 2 as EquipmentandVehicle_Cust,
# MAGIC 		cs.Loadandunload_Cust / 2 as Loadandunload_Cust,
# MAGIC 		cs.Miscellaneous_Cust / 2 as Miscellaneous_Cust,
# MAGIC 		cs.PermitsandCompliance_cust / 2 as PermitsandCompliance_cust,
# MAGIC 		ca.AdminFees_Carrier / 2 as AdminFees_Carrier,
# MAGIC 		ca.TransitandRouting_Carrier / 2 as TransitandRouting_Carrier,
# MAGIC 		ca.DelandPickup_Carrier / 2 as DelandPickup_Carrier,
# MAGIC 		ca.EquipmentandVehicle_Carrier / 2 as EquipmentandVehicle_Carrier,
# MAGIC 		ca.Loadandunload_Carrier / 2 as Loadandunload_Carrier,
# MAGIC 		ca.Miscellaneous_Carrier / 2 as Miscellaneous_Carrier,
# MAGIC 		ca.PermitsandCompliance_carrier / 2 as PermitsandCompliance_carrier
# MAGIC 	  from
# MAGIC 		final
...
# MAGIC %sql
# MAGIC use schema bronze;
# MAGIC  create or replace table analytics.vw_genai_fact_load_v1_Temp
# MAGIC 	WITH new_customer_money_sp AS (
# MAGIC 	  SELECT DISTINCT
# MAGIC 		m.relay_reference_number AS loadnum,
# MAGIC 		SUM(
...
# MAGIC %sql
# MAGIC
# MAGIC truncate table analytics.vw_genai_fact_load_v1;
# MAGIC INSERT INTO analytics.vw_genai_fact_load_v1
# MAGIC SELECT *
# MAGIC FROM analytics.vw_genai_fact_load_v1_Temp
