...
ErrorLogger = ErrorLogs("PostgreSQL_SourceToStage_Batch_24hrsNightlyRefresh_5")
...
DF_Metadata = spark.sql("SELECT * FROM Metadata.MasterMetadata WHERE Tableid in ('R1','E5','E6','A3','A5','E7','R14','R17','R24','R26','A8','R34','R41','A9','A11','S1', 'T2') and IsActive='1' ")
...
def process_timestamp(timestamp_str):
    ...
    # Try to parse the timestamp using the defined formats
    for fmt in formats:
        try:
    ...
    if '.' in timestamp_str:
        new_timestamp_str = new_timestamp.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
    else:
    ...
    try:
        ...
        if AutoSkipperCheck == 0:
            ...
            if LoadType == "incremental load":
                ...
                MaxLoadDate_Offset = process_timestamp(MaxLoadDate)  
