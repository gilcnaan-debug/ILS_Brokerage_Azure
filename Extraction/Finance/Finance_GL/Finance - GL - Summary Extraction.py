...
# MAGIC * **Description:** Extract the Summary files from the azure blob and load that into the Bronze Schema  GLSummary Table. 
...
ErrorLogger = ErrorLogs("Finance_Extraction_24hr_Refresh_GLSummary")
...
try: 
    ...
    folder_path = "Finance Prod/NFI_ILS_GLSummary_"
    ...
    summary_blobs = [result.name for result in blob_list]

    latest_blobs = []
    for blob_name in summary_blobs:
        timestamp_str = blob_name.split("_")[-1].split(".")[0]
        fiscal_year = blob_name.split("GLSummary_")[1].split("_")[0]
...
MergeQuery = '''
                    ...
                    USING VW_GLSummary AS VW ON GLD.{2}=VW.{2}
...
try:
    for blob_name, fiscal_year,timestamp in latest_blobs:
        ...
        existing_columns = ['OBJ_ID','COMPANY','ACCT_UNIT','ACCT_UNIT_DESCR','ACCT_SUB','ACCOUNT_DESC','ACCT_PERIOD','FISCAL_YEAR','CURRENCY_CODE','TRAN_AMOUNT', 'BASE_AMOUNT', 'RPT_AMOUNT_1','UNITS_AMOUNT']
        ...
        pd_df = df[['OBJ_ID','COMPANY','ACCT_UNIT','ACCT_UNIT_DESCR','ACCT_SUB','ACCOUNT_DESC','ACCT_PERIOD','FISCAL_YEAR','CURRENCY_CODE','TRAN_AMOUNT', 'BASE_AMOUNT', 'RPT_AMOUNT_1', 'UNITS_AMOUNT','SOURCE_FILE_NAME']]
        ...
        try:
            ...
            logger.info(f"Unable to create Hashkey for summary{e}")
            print(e)
        try:
                ...
                try:
                    ...
                    try:
                        ...
                        WHERE {2} IN (
                            SELECT GLD.{2}
                            FROM {0}.{1} GLD
                            Left JOIN VW_GLSummary VW ON GLD.{2} = VW.{2}
