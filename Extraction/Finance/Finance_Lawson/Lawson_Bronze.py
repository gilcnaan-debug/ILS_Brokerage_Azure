...
ErrorLogger = ErrorLogs("PostgreSQL_SourceToStage_Lawson_1hr_refresh")
...
try:
    ...
    username = dbutils.secrets.get(scope = "NFI_PostgreSQL_Secrets", key = "ReportingPostgresqlUN")
    password = dbutils.secrets.get(scope = "NFI_PostgreSQL_Secrets", key = "ReportingPostgresqlPass")
    hostname = dbutils.secrets.get(scope = "NFI_PostgreSQL_Secrets", key = "ReportingPostgresqlHN")
    databasename = dbutils.secrets.get(scope = "NFI_PostgreSQL_Secrets", key = "ReportingPostgresqlDBName")
...
DF_Metadata = spark.sql("SELECT * FROM Metadata.MasterMetadata WHERE Tableid in ('A5-1','L14') and IsActive='1' ")
Job_ID = "JOB_F11"
Notebook_ID = "NB47"
Zone = "Bronze"
