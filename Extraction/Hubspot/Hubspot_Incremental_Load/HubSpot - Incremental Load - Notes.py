...
# MAGIC * **Description:** To update deletes and archived from Source to Bronze table
...
ErrorLogger = ErrorLogs("HubSpot_Companies_SourceToBronze_Deletes")
...
try:
    ...
    if AutoSkipperCheck == 0:
        ...
        UPDATE 
            Bronze.{TableName_I1}
        SET 
            Bronze.{TableName_I1}.Is_Deleted = 1
        WHERE 
            Bronze.{TableName_I1}.{MergeKey_I1} 
            IN (SELECT Target.{MergeKey_I1} FROM SourceToBronze AS Source 
                RIGHT JOIN Bronze.{TableName_I1} as Target 
                ON Source.{MergeKey_I1} = Target.{MergeKey_I1}
                WHERE 
                Target.Is_Deleted = 0 AND Source.{MergeKey_I1} IS NULL)
        """

        ArchiveUpdateQuery = f"""
        MERGE INTO Bronze.{TableName_I1} AS Target
        USING SourceToBronze AS Source
        ON Target.{MergeKey_I1} = Source.{MergeKey_I1}
