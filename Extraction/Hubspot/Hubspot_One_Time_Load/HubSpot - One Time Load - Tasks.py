...
ErrorLogger = ErrorLogs("HubSpot_One_Time_Load_Meetings")
...
try: 
    Table_ID_I1 = 'H13'
    DF_Metadata_I1 = spark.sql("SELECT * FROM Metadata.MasterMetadata WHERE Tableid = 'H13' and IsActive='1'")
...
try: 
    Table_ID_I2 = 'H6'
    DF_Metadata_I2 = spark.sql("SELECT * FROM Metadata.MasterMetadata WHERE Tableid = 'H6' and IsActive='1'")
...
def get_all_properties(client):
    properties = []
    try:
        response = client.crm.properties.core_api.get_all(object_type='meetings')
...
def get_data_for_instance(client, max_load_date, all_properties, archived):
    ...
    try:
        ...
        while True:
            ...
            api_response = client.crm.objects.meetings.basic_api.get_page(
                limit=100,
