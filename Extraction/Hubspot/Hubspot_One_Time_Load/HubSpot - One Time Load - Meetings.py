...
import pytz
import math
from hubspot.crm.objects.communications import PublicObjectSearchRequest
...
ErrorLogger = ErrorLogs("HubSpot_One_Time_Load_Communications")
...
try: 
    Table_ID_I1 = 'H18'
    DF_Metadata_I1 = spark.sql("SELECT * FROM Metadata.MasterMetadata WHERE Tableid = 'H18' and IsActive='1'")
...
try: 
    Table_ID_I2 = 'H19'
    DF_Metadata_I2 = spark.sql("SELECT * FROM Metadata.MasterMetadata WHERE Tableid = 'H19' and IsActive='1'")
...
def get_all_properties(client):
    properties = []
    try:
        response = client.crm.properties.core_api.get_all(object_type='communications')
...
# Function to fetch data for an instance based on max_load_date and archived status
def get_data_for_instance(client, max_load_date, all_properties, archived):
    api_list = []
    next_page = None  # Initialize pagination token to None for the first request
    
    try:
        while True:
            time.sleep(0.5)  # To avoid hitting API rate limits, adjust as needed
            
            # Fetching data with the correct pagination token
            api_response = client.crm.objects.communications.basic_api.get_page(
                limit=100,
                archived=archived,  # Using the 'archived' parameter
                after=next_page if next_page else None,  # Properly handle pagination
                properties=all_properties
            )

            # Add the fetched items to api_list with additional properties
            ...
            try:
                next_page = api_response.paging.next.after  # Get the next page token
                if not next_page:
...
try:
    ...
    if AutoSkipperCheck == 0:
        ...
        if LoadType_I1 == "incremental load":
            ...
            UpdateLogStatus(Job_ID_I1, Table_ID_I1, Notebook_ID_I1, TableName_I1, Zone_I1, "Succeeded", "NULL", "NULL", 0, "Account Manager Merge")
            UpdateLogStatus(Job_ID_I2, Table_ID_I2, Notebook_ID_I2, TableName_I2, Zone_I2, "Succeeded", "NULL", "NULL", 0, "NFI Industries Merge")
    ...
    UpdateLogStatus(Job_ID_I1, Table_ID_I1, Notebook_ID_I1, TableName_I1, Zone_I1,"Failed",Error_Statement,"NotIgnorable",1,"Account Manager Merge")
    UpdateLogStatus(Job_ID_I2, Table_ID_I2, Notebook_ID_I2, TableName_I2, Zone_I2,"Failed",Error_Statement,"NotIgnorable",1,"NFI Industries Merge")
