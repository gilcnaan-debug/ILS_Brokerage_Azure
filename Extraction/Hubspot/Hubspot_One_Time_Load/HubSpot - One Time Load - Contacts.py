...
ErrorLogger = ErrorLogs("HubSpot_One_Time_Load_Calls")
...
try: 
    Table_ID_I1 = 'H10'
    DF_Metadata_I1 = spark.sql("SELECT * FROM Metadata.MasterMetadata WHERE Tableid = 'H10' and IsActive='1'")
...
try: 
    Table_ID_I2 = 'H15'
    DF_Metadata_I2 = spark.sql("SELECT * FROM Metadata.MasterMetadata WHERE Tableid = 'H15' and IsActive='1'")
...
def convert_to_millisec(date_str):
    try:
        ...
        dt_obj_minus_10mins = dt_obj - timedelta(minutes=10)
        
        # Subtract 10 minutes from the datetime object
        return math.floor(dt_obj_minus_10mins.timestamp() * 1000)  # Convert to milliseconds
...
def get_all_properties(client):
    properties = []
    try:
        response = client.crm.properties.core_api.get_all(object_type='calls')
...
try:
    ...
    if AutoSkipperCheck == 0:
        ...
        if LoadType_I1 == "incremental load":
            ...
            logger.info(f"Table {TableName_I1} has been successfully loaded to Bronze")
