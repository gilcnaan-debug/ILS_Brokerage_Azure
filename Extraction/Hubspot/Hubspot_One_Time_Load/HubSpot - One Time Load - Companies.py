...
ErrorLogger = ErrorLogs("HubSpot - One Time Load Deals")
...
try: 
    Table_ID_I1 = 'H9'
    DF_Metadata_I1 = spark.sql("SELECT * FROM Metadata.MasterMetadata WHERE Tableid = 'H9' and IsActive='1'")
...
try: 
    Table_ID_I2 = 'H1'
    DF_Metadata_I2 = spark.sql("SELECT * FROM Metadata.MasterMetadata WHERE Tableid = 'H1' and IsActive='1'")
...
def get_all_properties(client):
    properties = []
    try:
        response = client.crm.properties.core_api.get_all(object_type='deals')
...
def get_data_for_instance(client, all_properties, archived):
    api_list = []
    next_page = None  # Start with None for pagination
    ...
    try:
        ...
        while True:
            ...
            if not api_response.results:
                print(f"No results found for archived={archived}.")
                break
            ...
            next_page = api_response.paging.next.after if hasattr(api_response, 'paging') and api_response.paging else None
            
            if not next_page:  # Exit if no next page
                break
        
        print(f"Fetched {len(api_list)} items for archived={archived}.")
        return api_list
...
for i, access_token in enumerate(access_tokens):
    ...
    NonArchivedData = get_data_for_instance(client, list(all_properties_set), False)
    ArchivedData = get_data_for_instance(client, list(all_properties_set), True)


    if NonArchivedData is not None and ArchivedData is not None:
        SourceData = NonArchivedData + ArchivedData
    elif NonArchivedData is not None and ArchivedData is None:
...
if all_dfs:
    ...
    if 'archived_at' in combined_df.columns:
        ...
        combined_df['archived_at'] = combined_df['archived_at'].replace('NaT', None)
        combined_df['archived_at'] = combined_df['archived_at'].replace('None', None)
        combined_df['archived_at'] = combined_df['archived_at'].replace('null', None)
...
try:
    ...
    if AutoSkipperCheck == 0:
        ...
        if LoadType_I1 == "incremental load":
            ...
            UpdateLogStatus(Job_ID_I1, Table_ID_I1, Notebook_ID_I1, TableName_I1, Zone_I1, "Succeeded", "NULL", "NULL", 0, "Acccount Manager")
