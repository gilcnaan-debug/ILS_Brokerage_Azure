    ...
    try:
        ...
        if AutoSkipperCheck == 0:
            ...
            if LoadType == "incremental load":
                ...
                DF_Source_Load = DF_Source_Load.withColumn('Active', lit(1))
                DF_Source_Load.createOrReplaceTempView('SourceToBronze')
                Rowcount = DF_Source_Load.count()
                MergeQuery = 'Merge into bronze.pricing_{0} Target using SourceToBronze source on Target.HashKey = Source.HashKey  WHEN MATCHED THEN UPDATE SET * WHEN NOT MATCHED THEN INSERT *'
                spark.sql(MergeQuery.format(TableName, MergeKey, MergeKey))
                UpdateQuery = """
                    UPDATE bronze.pricing_{0} AS Target
                    SET Active = CASE
                        WHEN EXISTS (SELECT 1 FROM SourceToBronze WHERE SourceToBronze.Hashkey = Target.Hashkey) THEN 1
                        ELSE 0
                    END
                """
                spark.sql(UpdateQuery.format(TableName))
                logger.info('Successfully loaded the ' + TableName + ' to bronze')
              
                print('Loaded ' + TableName)
                MaxDateQuery = "Select max({0}) as Max_Date from bronze.pricing_{1}"
