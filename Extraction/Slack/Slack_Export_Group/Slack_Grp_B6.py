...
try:
    ErrorLogger = ErrorLogs("Slack_conversation_History_MPDM_B6")
...
try:
    DF_Metadata= spark.sql(
        "SELECT * FROM Metadata.MasterMetadata where TableID='SGC-B6' and IsActive='1' "
)
except Exception as e:
...
try:
    TableID  =  (DF_Metadata.select(col("TableID")).where(col("TableID") == "SGC-B6").collect()[0].TableID)
except Exception as e:
    logger.info("Unable to fetch details",e)
    print(e)
    ...
    try:
        ...
        if AutoSkipperCheck == 0:
            ...
            try:
                ...
                MergeQuery = '''
                    MERGE INTO bronze.Slack_Temp_Grp6 AS Target
                ...
                # dg=spark.sql('''MERGE INTO bronze.Slack_Temp_Grp6 AS conversations
                #                 USING (
                #                     SELECT name, id
                #                     FROM (
                #                         SELECT name,
...
start_index = 5*batch_size
end_index = builtins.min(6*batch_size, total_folders) #Set this to desired range or len(sorted_folders) for all
...
# MAGIC %md
# MAGIC ##Update Channel_Id into table
