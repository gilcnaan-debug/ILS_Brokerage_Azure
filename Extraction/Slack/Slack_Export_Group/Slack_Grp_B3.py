...
try:
    ErrorLogger = ErrorLogs("Slack_conversation_History_MPDM_B3")
...
try:
    DF_Metadata= spark.sql(
        "SELECT * FROM Metadata.MasterMetadata where TableID='SGC-B3' and IsActive='1' "
)
except Exception as e:
...
try:
    TableID  =  (DF_Metadata.select(col("TableID")).where(col("TableID") == "SGC-B3").collect()[0].TableID)
except Exception as e:
    logger.info("Unable to fetch details",e)
    print(e)
    ...
    try:
        ...
        if AutoSkipperCheck == 0:
            ...
            try:
                ...
                MergeQuery = '''
                    MERGE INTO bronze.Slack_Temp_Grp3 AS Target
                ...
                # dg=spark.sql('''MERGE INTO bronze.Slack_Temp_Grp3 AS conversations
                #                 USING (
                #                     SELECT name, id
                #                     FROM (
                #                         SELECT name,
...
start_index = 2*batch_size
end_index = builtins.min(3*batch_size, total_folders) #Set this to desired range or len(sorted_folders) for all
...
# MAGIC %md
# MAGIC ##Update channel_ids into table
