...
try:
    ErrorLogger = ErrorLogs("Slack_conversation_History_MPDM_b1")
...
try:
    DF_Metadata= spark.sql(
        "SELECT * FROM Metadata.MasterMetadata where TableID='SGC-B1' and IsActive='1' "
)
except Exception as e:
...
try:
    TableID  =  (DF_Metadata.select(col("TableID")).where(col("TableID") == "SGC-B1").collect()[0].TableID)
except Exception as e:
    logger.info("Unable to fetch details",e)
    print(e)
...
# MAGIC %md
# MAGIC ##RETRIEVE CHANNEL_ID FROM FILE
# MAGIC
    ...
    try:
        ...
        if AutoSkipperCheck == 0:
            ...
            try:
                ...
                MergeQuery = '''
                    MERGE INTO bronze.Slack_Temp_Grp1 AS Target
                ...
                # dg=spark.sql('''MERGE INTO bronze.Slack_Temp_Grp1 AS conversations
                #                 USING (
                #                     SELECT name, id
                #                     FROM (
                #                         SELECT name,
...
start_index = 0
end_index = builtins.min(batch_size, total_folders) #Set this to desired range or len(sorted_folders) for all
