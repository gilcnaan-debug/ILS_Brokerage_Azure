...
try:
    ErrorLogger = ErrorLogs("Slack_conversation_History_MPDM_B2")
...
try:
    DF_Metadata= spark.sql(
        "SELECT * FROM Metadata.MasterMetadata where TableID='SGC-B2' and IsActive='1' "
)
except Exception as e:
...
try:
    TableID  =  (DF_Metadata.select(col("TableID")).where(col("TableID") == "SGC-B2").collect()[0].TableID)
except Exception as e:
    logger.info("Unable to fetch details",e)
    print(e)
    ...
    try:
        ...
        if AutoSkipperCheck == 0:
            ...
            try:
                ...
                MergeQuery = '''
                    MERGE INTO bronze.Slack_Temp_Grp2 AS Target
                ...
                # dg=spark.sql('''MERGE INTO bronze.Slack_Temp_Grp2 AS conversations
                #                 USING (
                #                     SELECT name, id
                #                     FROM (
                #                         SELECT name,
