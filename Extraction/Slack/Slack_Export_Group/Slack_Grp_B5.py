...
# MAGIC * **Created Date:** 01/10/2024
# MAGIC * **Created By:** Gagana Nair
# MAGIC * **Modified Date:** 01/10/2024
...
try:
    ErrorLogger = ErrorLogs("Slack_conversation_History_MPDM_B5")
...
try:
    DF_Metadata= spark.sql(
        "SELECT * FROM Metadata.MasterMetadata where TableID='SGC-B5' and IsActive='1' "
)
except Exception as e:
...
try:
    TableID  =  (DF_Metadata.select(col("TableID")).where(col("TableID") == "SGC-B5").collect()[0].TableID)
except Exception as e:
    logger.info("Unable to fetch details",e)
    print(e)
...
# MAGIC %md
# MAGIC ##Log Table Variables

# COMMAND ----------

try:
    Job_ID = "Sl_Grp_5"
    Notebook_ID = "SlNB_Grp_5"
    Zone = "Bronze"
    Table_ID=TableID
...
# MAGIC %md
# MAGIC ## Establish connection to Azure container
...
# MAGIC %md
# MAGIC ##Retrieve Channel_Ids from File
...
# Variable to store the content of mpims.json
mpims_json_content = None

with zipfile.ZipFile(zip_data, 'r') as zip_ref:
    for file_name in zip_ref.namelist():
        if file_name.endswith('mpims.json'):
            with zip_ref.open(file_name) as mpims_file:
                mpims_json_content = mpims_file.read().decode('utf-8')
            break  # Stop searching once we've found and read the file

if mpims_json_content:
    # Parse the JSON string into a Python object
    import json
    mpims_data = json.loads(mpims_json_content)
    ...
    DF_Messages = spark.createDataFrame(mpims_data, schema)
    DF_Messages.createOrReplaceTempView("VW_Channels")

    print("mpims.json content found, parsed, and stored in DataFrame.")
else:
    print("mpims.json not found in the zip file.")

# You can now use DF_Messages for further processing

# COMMAND ----------

# MAGIC %md
# MAGIC ##Retrieve folders starting with MPDM
...
# Extract folders
folders_starting_with_mpdm = set()

with zipfile.ZipFile(zip_data, 'r') as zip_ref:
    for entry in zip_ref.infolist():
        # Extract all parts of the path, which could indicate a folder
        path_parts = entry.filename.split('/')
        # We check each part for a folder that starts with 'mpdm'
        for part in path_parts:
            if part.startswith('mpdm'):
                # If the part starts with 'mpdm', we consider it a folder and add it to our set
                folders_starting_with_mpdm.add('/'.join(path_parts[:path_parts.index(part) + 1]))

# The folders are already unique because we used a set, now we just sort them
sorted_folders_starting_with_mpdm = sorted(folders_starting_with_mpdm)

print(len(sorted_folders_starting_with_mpdm))

# COMMAND ----------

# MAGIC %md
# MAGIC ## Retrieve folders and merge into table
    ...
    for zip_info in folder_files:
        with zip_ref.open(zip_info) as file:
            ...
            try:
                ...
                for message in data:
                    if message.get("subtype") != "bot_message" and message.get("username") != "offerbot":
                        message["name"] = folder_name
                        yield message
            except Exception as e:
...
def merge_data(df, table_name, table_id):
    try:
        ...
        if AutoSkipperCheck == 0:
            ...
            df = df.withColumn("Channel_ID", lit("None"))
            ...
            df = df.withColumn("Conversation_Type", lit("Group Message"))
            ...
            try:
                ...
                MergeQuery = '''
                    MERGE INTO bronze.Slack_Temp_Grp5 AS Target
                ...
                # dg=spark.sql('''MERGE INTO bronze.Slack_Temp_Grp5 AS conversations
                #                 USING (
                #                     SELECT name, id
                #                     FROM (
                #                         SELECT name,
                ...
                #                         FROM bronze.slack_channels_new
                #                     ) tmp
                #                     WHERE rn = 1
                #                 ) AS channels
                #                 ON conversations.name = channels.name
                #                 WHEN MATCHED THEN 
                #                     UPDATE SET conversations.Channel_ID = channels.id''')
...
zip_data = BytesIO(zip_blob_data)
total_folders = len(sorted_folders_starting_with_mpdm)
# Calculate batch size
batch_size = -(-total_folders // 6)
start_index = 4*batch_size
end_index = builtins.min(5*batch_size, total_folders) #Set this to desired range or len(sorted_folders) for all
second_batch = sorted_folders_starting_with_mpdm[start_index:end_index]
print(f"Total folders: {total_folders}")
print(f"Batch size: {batch_size}")
print(f"Processing folders from index {start_index} to {end_index}")
# DF_Channels=spark.sql(''' select id,name from bronze.slack_channels_new where Conversation_Type = 'Group Message' ''')
...
# MAGIC %md
# MAGIC ##Update Channel_Id into the Table
