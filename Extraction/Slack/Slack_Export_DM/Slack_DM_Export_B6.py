...
try:
    ErrorLogger = ErrorLogs("Slack_Export_DM_B6_test")
...
try:
    DF_Metadata= spark.sql(
        "SELECT * FROM Metadata.MasterMetadata where TableID='SCD-B6' and IsActive='1' "
)
except Exception as e:
...
try:
    TableID  =  (DF_Metadata.select(col("TableID")).where(col("TableID") == "SCD-B6").collect()[0].TableID)
except Exception as e:
    logger.info("Unable to fetch details",e)
    print(e)
    ...
    try:
        ...
        if AutoSkipperCheck == 0:
            ...
            try:
                ...
                MergeQuery = '''
                    MERGE INTO bronze.Slack_Temp_DM6 AS Target
