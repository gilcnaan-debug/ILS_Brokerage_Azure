...
try:
    ErrorLogger = ErrorLogs("Slack_conversation_History's_Grp_P4")
...
try:
    ...
    print("Unable to retrieve metadata details",e)
    print(e)
...
DF_Query = spark.sql('''select id from bronze.temp_channels_Grp
where conversation_type = "Group Message" 
and partition between 15 and 19 And Is_Channel_Deleted=0 ''')
DF_Query.count()
...
# MAGIC %md
# MAGIC ##Extract Data from slack API
    ...
    for channel_row in DF_Query.collect():
        ...
        while True:
            ...
            try:
                ...
                UpdateLogStatus(Job_ID,Table_ID,Notebook_ID,Table_Name,Zone,"Failed",Error_Statement,"Ignorable",1,"Slack-Group_Message")
                ...
                if e.response.get("headers", {}).get("Retry-After"):
                    ...
                    continue
        UpdateLogStatus(Job_ID,Table_ID,Notebook_ID,Table_Name,Zone,"Succeeded","NULL","NULL",0,"Slack-Group_Message")
except Exception as e:
    logger.info("Unable to get data from slack API")
    print(e)
