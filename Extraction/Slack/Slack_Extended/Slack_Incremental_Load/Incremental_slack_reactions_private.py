...
#Creating variables to store error log information
try:
    ErrorLogger = ErrorLogs("Slack_Reactions_log_private")
...
try:
    Job_ID = "SJOB_4_private_incremental"
    Notebook_ID = "SNB_4_private_incremental"
    Zone = "Bronze"
    Table_ID=TableID
...
try:
  MaxDateQuery = "Select max({0}) as Max_Date from bronze.slack_Reactions_New where Conversation_Type = 'Private' "
...
try:
    ...
    DF_Query = f"select distinct(id), context_team_id from bronze.slack_Channels_new where Conversation_Type = 'Private' and Is_Channel_Deleted = 0 and last_activity_ts > '{max_time}'"
    DF_Query=spark.sql(DF_Query)
    DF_Query.count()
except Exception as e:
  logger.info("Unable to fetch channels ids",e)
  print(e)
...
for channel_row in DF_Query.collect():
    ...
    while True:
        ...
        try:
            ...
            UpdateLogStatus(Job_ID,Table_ID,Notebook_ID,Table_Name,Zone,"Failed",Error_Statement,"Ignorable",1,"Slack-reactions-private")
            ...
            if e.response.get("headers", {}).get("Retry-After"):
                ...
                continue
    UpdateLogStatus(Job_ID,Table_ID,Notebook_ID,Table_Name,Zone,"Succeeded","NULL","NULL",0,"Slack-reactions-private")
...
try:
    ...
    logger.info("Unable to pharse the columns into schema",e)
    print(e)
...
try:
    ...
    Hash_Key = ["team","Date_Create","Channel_ID"]
