...
try:
    ErrorLogger = ErrorLogs("Slack_conversation_History_DM")
...
try:
    Job_ID = "SJOB_3_DM_incremental"
    Notebook_ID = "NB_3_DM_Incremental"
    Zone = "Bronze"
    Table_ID=TableID
...
try:
    MaxDateQuery = "Select max({0}) as Max_Date from bronze.slack_Conversations_History_New where Conversation_Type = 'Direct Message' "
...
try:
    DF_Query = spark.sql(''' select id from bronze.slack_Channels_new where Conversation_Type = 'Direct Message' and Is_Channel_Deleted = 0''')
except Exception as e:
    logger.info("Unable to fetch channels details",e)
    print(e)
    ...
    while True:
        ...
        try:
            ...
            if channels_response.get('ok', False):
                ...
                UpdateLogStatus(Job_ID,Table_ID,Notebook_ID,Table_Name,Zone,"Failed",Error_Statement,"Ignorable",1,"Slack-DM")
                ...
                if e.response.get("headers", {}).get("Retry-After"):
                    ...
                    continue
    UpdateLogStatus(Job_ID,Table_ID,Notebook_ID,Table_Name,Zone,"Succeeded","NULL","NULL",0,"Slack-DM")
