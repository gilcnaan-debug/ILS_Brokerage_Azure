...
try:
    ErrorLogger = ErrorLogs("Slack_Reactions_log_DM")
...
try:
    Job_ID = "SJOB_4_DM_incremental"
    Notebook_ID = "SNB_4_DM_incremental"
    Zone = "Bronze"
    Table_ID=TableID
...
try:
    ...
    logger.info("Unable to fetch channel ids",e)
    print(e)
...
for channel_row in DF_Query.collect():
    ...
    while True:
        ...
        try:
            ...
            UpdateLogStatus(Job_ID,Table_ID,Notebook_ID,Table_Name,Zone,"Failed",Error_Statement,"Ignorable",1,"Slack-reactions-DM")
            ...
            if e.response.get("headers", {}).get("Retry-After"):
                ...
                continue
    UpdateLogStatus(Job_ID,Table_ID,Notebook_ID,Table_Name,Zone,"Succeeded","NULL","NULL",0,"Slack-reactions-DM")
...
try:
    ...
    logger.info("Unable to pharse the columns ",e)
    print(e)
