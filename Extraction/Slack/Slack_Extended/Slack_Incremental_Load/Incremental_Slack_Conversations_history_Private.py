...
try:
    ErrorLogger = ErrorLogs("Slack_conversation_History_log_private_incremental")
...
try:
    Job_ID = "SJOB_3_Private_incremental"
    Notebook_ID = "SNB_3_Private_incremental"
    Zone = "Bronze"
    Table_ID=TableID
...
try:
    MaxDateQuery = "Select max({0}) as Max_Date from bronze.slack_Conversations_History_New where Conversation_Type = 'Private' "
...
# MAGIC %md
# MAGIC # Retrieveing Channel ID's

# COMMAND ----------

try:
    max_time = DF_MaxDate.select(col("Max_Date")).collect()[0]["Max_Date"]
    DF_Query = f"select id, context_team_id from bronze.slack_Channels_new where Conversation_Type = 'Private' and Is_Channel_Deleted = 0 and last_activity_ts >'{max_time}' "
    DF_Query=spark.sql(DF_Query)
except Exception as e:
    logger.info("unable to get Channels ids")
    print(e)
    ...
    while True:
        ...
        try:
            ...
            if channels_response.get('ok', False):
                ...
                UpdateLogStatus(Job_ID,Table_ID,Notebook_ID,Table_Name,Zone,"Failed",Error_Statement,"Ignorable",1,"Slack-Private")
                ...
                if e.response.get("headers", {}).get("Retry-After"):
                    ...
                    continue
    UpdateLogStatus(Job_ID,Table_ID,Notebook_ID,Table_Name,Zone,"Succeeded","NULL","NULL",0,"Slack-private")
