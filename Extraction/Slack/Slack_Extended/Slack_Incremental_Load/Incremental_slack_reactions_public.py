...
try:
    ErrorLogger = ErrorLogs("Slack_Reactions_log_public_incremental")
...
try:
    Job_ID = "SJOB_4_public_incremental"
    Notebook_ID = "SNB_4_public_incremental"
    Zone = "Bronze"
    Table_ID=TableID
...
try:
    ...
    DF_Query = f" select distinct(id), context_team_id from bronze.slack_Channels_new where Conversation_Type = 'Public' and Is_Channel_Deleted = 0 and last_activity_ts > '{max_time}' "
    DF_Query=spark.sql(DF_Query)
except Exception as e:
    logger.info("Unable to get channel ids")
    print(e)
...
for channel_row in DF_Query.collect():
    ...
    while True:
        ...
        try:
            ...
            UpdateLogStatus(Job_ID,Table_ID,Notebook_ID,Table_Name,Zone,"Failed",Error_Statement,"Ignorable",1,"Slack-reactions-public")
            ...
            if e.response.get("headers", {}).get("Retry-After"):
                ...
                continue
    UpdateLogStatus(Job_ID,Table_ID,Notebook_ID,Table_Name,Zone,"Succeeded","NULL","NULL",0,"Slack-reactions-public")
...
try:
    ...
    logger.info("Unable to parse the columns",e)
    print(e)
