...
# MAGIC * **Created Date:** 10/22/2023
# MAGIC * **Created By:** Gagana Nair
# MAGIC * **Modified Date:** 10/22/2023
...
ErrorLogger = ErrorLogs("PostgreSQL_SourceToStage_NFI_Spot_Extraction_log")
...
# MAGIC %md
# MAGIC ##Retrieve Metadata Details

# COMMAND ----------

#Getting the list of DMS tables in the dataframe that needs to be loaded into stage
DF_Metadata = spark.sql("SELECT * FROM Metadata.MasterMetadata WHERE Tableid in ('SQ-1') and IsActive='1' ")
    ...
    try:
        ...
        if AutoSkipperCheck == 0:
            ...
            if LoadType == "incremental load":
                ...
                DF_Source_Load = DF_Source_Load.withColumn("Sourcesystem_ID", lit(10))
                DF_Source_Load = DF_Source_Load.withColumn("Sourcesystem_Name", lit("Spot"))
