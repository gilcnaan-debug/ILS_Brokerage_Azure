...
ErrorLogger = ErrorLogs("Silver_slack_Conversaions_History_DM_Log")
...
try:
    DF_Bronze_History = spark.sql(
                    '''select 
                        ...
                        from bronze.Slack_Conversations_History_DM ch
                         '''
                )
except Exception as e:
...
try:
    ...
    if AutoSkipperCheck == 0:
        try:
            ...
            MergeQuery = '''
                MERGE INTO silver.Silver_Slack_Conversations_History_DM AS Target
            ...
            Reaction = spark.sql('''UPDATE silver.Silver_Slack_Conversations_History_DM AS s
                                SET s.Is_Reactions = CASE 
                                WHEN s.Reactions IS NOT NULL THEN 1 
                                ELSE 0  END''')
            
            DF_Workspace=spark.sql('''
                UPDATE silver.Silver_Slack_Conversations_History_DM wc 
                SET 
