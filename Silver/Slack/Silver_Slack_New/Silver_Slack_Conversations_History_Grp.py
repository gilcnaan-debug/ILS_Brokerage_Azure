...
ErrorLogger = ErrorLogs("Silver_slack_Conversaions_History_MPDM_Log")
...
try:
    DF_Bronze_History = spark.sql(
                    '''select 
                        ...
                        CASE WHEN ch.Reactions is not null THEN '1' ELSE '0' END AS Is_Reactions,
                        ...
                        from bronze.Slack_Conversations_History_Groups ch
                         '''
                )
except Exception as e:
...
try:
    ...
    if AutoSkipperCheck == 0:
        try:
            ...
            MergeQuery = '''
                MERGE INTO silver.Silver_Slack_Conversations_History_Groups AS Target
                ...
                UPDATE SET
                    Target.DW_Conversations_History_ID = Source.DW_Conversations_History_ID,
                        ...
                        Target.Is_Reactions = Source.Is_Reactions,
            ...
            Reaction = spark.sql('''UPDATE silver.Silver_Slack_Conversations_History_Groups AS s
                                SET s.Is_Reactions = CASE 
                                WHEN s.Reactions IS NOT NULL THEN 1 
                                ELSE 0  END''')
            
            DF_Workspace=spark.sql('''
                UPDATE silver.Silver_Slack_Conversations_History_Groups wc 
                SET 
