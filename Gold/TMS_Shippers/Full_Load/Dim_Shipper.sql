...
MERGE INTO Gold.Dim_Shipper AS tgt
USING Gold.VW_Dim_Shipper AS src
ON tgt.MergeKey = src.MergeKey
WHEN MATCHED AND tgt.Hashkey != src.HashKey THEN
    UPDATE SET
        tgt.DW_Shipper_ID = src.DW_Shipper_ID,
...
UPDATE metadata.mastermetadata
SET LastLoadDateValue = (SELECT MAX(Last_Modified_Date) FROM gold.dim_shipper where Is_Deleted = 0),
