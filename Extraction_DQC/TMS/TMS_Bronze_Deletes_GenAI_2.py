...
ErrorLogger = ErrorLogs("PostgreSQL_SourceToStage_Batch_15MinsRefresh_11")
...
##Based on the load type the loads will happen
    ...
    ##Generate Haskey value for the respective table based on the Unique column which is specified in the Metadata table 
            ...
            if LoadType == "incremental load":
                ...
                Max_Update = spark.sql(f" update metadata.dqmetadata set LastLoadDate = '{DQ_Max}' where Table_ID = '{TableID}'")
                LoadDateDel = "1999-01-01"
                ExtractionQueryDel = ExtractionQuery+"'"+LoadDateDel+"'"
