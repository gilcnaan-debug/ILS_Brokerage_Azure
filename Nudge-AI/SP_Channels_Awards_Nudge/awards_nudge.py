...
# MAGIC %md
# MAGIC Awards Nudge - SP channels Nudge
...
def logFailure(logs, reason, financial_date_result):
    ...
    try:
        ...
        period_year = f"{financial_period_number}-{financial_year}"
...
def automate_awards_nudge(dbutils):
    ...
    try:
        ...
        nudgeSqlQuery = f"""
        WITH prev_period AS (
        SELECT LAG(period_year) OVER (ORDER BY financial_period_startdate) AS prev_period_year
        FROM analytics.dim_date
        WHERE period_year IS NOT NULL
        AND financial_period_startdate <= current_date()
        QUALIFY ROW_NUMBER() OVER (ORDER BY financial_period_startdate DESC) = 1
        ) 
        SELECT DISTINCT * FROM nudge_ai.awards_slack_sam_mappings WHERE UPPER(shipper_name) IN (SELECT UPPER(CUSTOMER_MASTER) FROM Gold.fact_awards_summary WHERE Period_Year IN (SELECT prev_period_year FROM prev_period) AND Is_Deleted = 0 )
        """
